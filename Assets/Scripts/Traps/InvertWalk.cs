...
public class InvertWalk : MonoBehaviour
{
    void OnTriggerEnter2D (Collider2D check) {
        if (check.tag == "Player")
        {
            ...
            check.gameObject.GetComponent<PlayerController>().jumpForce = -16f;
            check.gameObject.GetComponent<PlayerController>().doubleJumpForce = -12f;
        }
    }
    ...
    void OnTriggerExit2D(Collider2D leaf) {
        if (leaf.tag == "Player")
        {
            ...
            leaf.gameObject.GetComponent<PlayerController>().jumpForce = 16f;
            leaf.gameObject.GetComponent<PlayerController>().doubleJumpForce = 12f;
        }
    }
}
